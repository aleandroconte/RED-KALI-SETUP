...
# Proxy globale: letto una volta sola all'avvio
PROXY: str = ""
...
def _read_proxy_from_zshrc() -> str:
    """Legge http_proxy o HTTP_PROXY dal .zshrc dell'utente corrente."""
    zshrc = Path.home() / ".zshrc"
    if not zshrc.exists():
        return ""
    for line in zshrc.read_text().splitlines():
        line = line.strip()
        # Cerca righe tipo: export http_proxy="http://..."
        m = re.match(r'^export\s+(?:http_proxy|HTTP_PROXY)\s*=\s*["\']?([^"\']+)["\']?', line)
        if m:
...
def ask_proxy() -> str:
    """
    Rileva il proxy in quest'ordine:
      1. Già impostato in questa sessione (riuso)
      2. Variabile d'ambiente http_proxy / HTTP_PROXY già presente
      3. Lettura da ~/.zshrc (dove set_proxy.sh lo scrive)
      4. Input manuale dall'utente
    """
    global PROXY
    if PROXY:
        return PROXY

    # 1. Variabile d'ambiente già presente
    for var in ("http_proxy", "HTTP_PROXY"):
        val = os.environ.get(var, "").strip()
        if val:
            PROXY = val
            log(f"Proxy rilevato dall'environment: {PROXY}")
            return PROXY

    # 2. Lettura da .zshrc
    val = _read_proxy_from_zshrc()
    if val:
        PROXY = val
        log(f"Proxy rilevato da ~/.zshrc: {PROXY}")
        # Inietta nell'environment del processo corrente così i figli lo ereditano
    ...
    # 3. Input manuale
    print(f"\n{BOLD}Proxy HTTP{RESET} (es: http://10.80.254.254:3128 — lascia vuoto se non serve)")
    val = input("  Inserisci il proxy: ").strip()
    PROXY = val
    if PROXY:
        for var in ("http_proxy", "https_proxy", "ftp_proxy",
                    "HTTP_PROXY", "HTTPS_PROXY", "FTP_PROXY"):
            os.environ[var] = PROXY
        log(f"Proxy impostato: {PROXY}")
    else:
        warn("Nessun proxy configurato. I download potrebbero fallire se sei dietro proxy.")
    return PROXY

def proxy_env() -> dict:
    """Restituisce un env dict con le variabili proxy impostate."""
    env = os.environ.copy()
    if PROXY:
        for var in ("http_proxy", "https_proxy", "ftp_proxy",
                    "HTTP_PROXY", "HTTPS_PROXY", "FTP_PROXY"):
...
def curl(url: str, dest: str):
    """Esegue curl con proxy esplicito (-x) se configurato."""
    proxy_flag = f'-x "{PROXY}"' if PROXY else ""
    run(f'curl -L {proxy_flag} -o {dest} {url}')

def run(cmd: str, check=True) -> subprocess.CompletedProcess:
    """Esegue un comando shell con output live, proxy iniettato nell'env."""
...
def run_list(cmd: list, check=True, cwd=None) -> subprocess.CompletedProcess:
    """Esegue un comando come lista con output live, proxy iniettato nell'env."""
...
def run_sudo(cmd: str, password: str, check=True) -> subprocess.CompletedProcess:
    """Esegue un comando con sudo -SE (env + proxy preservati), password via stdin, output live."""
    # -E preserva l'environment (proxy incluso), -S legge la password da stdin
    print(f"  {YELLOW}${RESET} sudo {cmd}")
    process = subprocess.Popen(
        f"sudo -SE {cmd}", shell=True, text=True,
...
def run_sudo_E(cmd: str, password: str, check=True) -> subprocess.CompletedProcess:
    """Esegue un comando con sudo -SE sh -c (env + proxy preservati), password via stdin."""
...
def sliver_download_and_fix():
    ...
    curl(SLIVER_INSTALL_URL, SLIVER_INSTALL_SCRIPT)
...
def _ensure_pexpect():
    """Installa pexpect se non disponibile."""
    try:
        import pexpect  # noqa
        log("pexpect già disponibile.")
    except ImportError:
        log("pexpect non trovato. Lo installo...")
        result = subprocess.run(
            [sys.executable, "-m", "pip", "install", "pexpect"],
        ...
        for line in result.stdout.strip().splitlines():
            print(f"    {line}")
        if result.returncode != 0:
            err("Installazione pexpect fallita. Installalo manualmente: pip install pexpect")
            sys.exit(1)
        log("pexpect installato.")

def sliver_armory(proxy: str):
    import importlib
    section("STEP 4 — Installazione pacchetti armory (via sliver CLI)")
    ...
    log(f"Avvio sliver CLI: {sliver_bin}")
    log("Attendo il prompt sliver >  (può richiedere qualche secondo al primo avvio)...")

    PROMPT        = "sliver >"
    TIMEOUT_START = 60   # secondi per attendere il primo prompt
    TIMEOUT_CMD   = 180  # secondi per ogni installazione armory
    ...
    try:
        ...
        child.logfile_read = sys.stdout   # mostra output live

        # Attende il prompt iniziale
        child.expect(PROMPT, timeout=TIMEOUT_START)
        log("Sliver pronto.")

        for pkg in ARMORY_PACKAGES:
            log(f"Installo armory: {pkg} ...")
            child.sendline(f"armory install {pkg} -p {proxy}")

            # armory chiede conferma [Y/n] — mandiamo Y
            idx = child.expect(
                [r"\[Y/n\]", r"\[y/N\]", PROMPT],
                timeout=30
            )
        ...
        log("Uscita da sliver...")
        child.sendline("exit")
        child.expect(pexpect.EOF, timeout=15)
        log("Sliver chiuso.")

    except pexpect.TIMEOUT:
        err("Timeout durante l'interazione con sliver.")
        warn("Esegui manualmente:\n"
             f"  sliver\n"
             f"  armory install mimikatz -p {proxy}\n"
             f"  armory install rubeus   -p {proxy}")
    except pexpect.EOF:
        err("Sliver si è chiuso inaspettatamente.")
        warn("Esegui manualmente:\n"
             f"  sliver\n"
             f"  armory install mimikatz -p {proxy}\n"
             f"  armory install rubeus   -p {proxy}")
    except Exception as e:
        err(f"Errore inatteso: {e}")
        warn("Esegui manualmente:\n"
             f"  sliver\n"
             f"  armory install mimikatz -p {proxy}\n"
             f"  armory install rubeus   -p {proxy}")
...
def install_xrdp():
    ...
    warn("apt install xrdp può bloccarsi su xorgxrdp: lo script rileva il blocco e applica il fix automaticamente.")

    # Lancia apt install xrdp con un timeout: se si blocca lo killamo e passiamo al fix
    XRDP_TIMEOUT = 180  # secondi — aumenta se la rete è lenta
    log(f"Timeout impostato a {XRDP_TIMEOUT}s (3 minuti). Se apt si blocca prima, verrà interrotto automaticamente e verrà applicato il fix.")

    proc = subprocess.Popen(
        f"echo '{password}' | sudo -S apt install xrdp -y",
        shell=True, text=True,
        stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
        env=proxy_env()
    )
    try:
        stdout, _ = proc.communicate(timeout=XRDP_TIMEOUT)
        for line in (stdout or "").splitlines():
            print(f"    {line}")
        if proc.returncode == 0:
            log("apt install xrdp completato senza blocchi.")
        else:
            warn(f"apt install xrdp uscito con codice {proc.returncode}. Applico fix...")
    except subprocess.TimeoutExpired:
        warn(f"apt install xrdp bloccato dopo {XRDP_TIMEOUT}s. Interrompo e applico fix...")
        proc.kill()
        proc.communicate()  # svuota i pipe
        # Termina eventuali processi apt rimasti appesi
        subprocess.run(
            f"echo '{password}' | sudo -S killall apt apt-get 2>/dev/null || true",
            shell=True, env=proxy_env()
        )

    # Fix: sempre eseguito, sia in caso di blocco che di errore
    log("Applico fix: apt install xorgxrdp --fix-missing ...")
  ...
  {GREEN}✔{RESET} xrdp installato (con fix xorgxrdp --fix-missing)
...
def install_impacket():
    ...
    # Ricava la versione esatta (es. "3.11") per il pacchetto python3.x-venv
    ...
    # Controlla se venv funziona; se no installa python3.x-venv automaticamente
    r = subprocess.run(["python3", "-m", "venv", "--help"], capture_output=True, text=True)
    if r.returncode != 0:
        warn(f"Modulo venv non disponibile. Installo python{py_ver}-venv automaticamente...")
        password = ask_sudo_password("necessaria per installare python-venv")
        run_sudo(f"apt install -y python{py_ver}-venv", password)
        # Riverifica
        r = subprocess.run(["python3", "-m", "venv", "--help"], capture_output=True, text=True)
        if r.returncode != 0:
            err("Installazione python-venv fallita. Installalo manualmente e riprova.")
            sys.exit(1)
...
def install_dotnet():
    ...
    curl(DOTNET_SCRIPT_URL, DOTNET_SCRIPT)
    run(f"chmod +x {DOTNET_SCRIPT}")
    log("Script scaricato.")

    # STEP 3: Installazione — passa il proxy allo script se configurato
    ...
    proxy_flag = f'https_proxy="{PROXY}" http_proxy="{PROXY}"' if PROXY else ""

    # Prima prova con --channel 10.0, poi con --quality preview come fallback
    result = run_sudo(
        f"env {proxy_flag} {DOTNET_SCRIPT} --channel {DOTNET_VERSION} --install-dir {DOTNET_INSTALL_DIR}",
        password, check=False
    )
    if result.returncode != 0:
        warn("Canale 10.0 non risolto. Provo con --channel 10.0 --quality preview ...")
        result = run_sudo(
            f"env {proxy_flag} {DOTNET_SCRIPT} --channel {DOTNET_VERSION} --quality preview --install-dir {DOTNET_INSTALL_DIR}",
            password, check=False
        )
    if result.returncode != 0:
        warn("Provo con --version 10.0.100 (versione esplicita) ...")
        run_sudo(
            f"env {proxy_flag} {DOTNET_SCRIPT} --version 10.0.100 --install-dir {DOTNET_INSTALL_DIR}",
            password
        )
...
def main():
    banner()
    ask_proxy()   # chiesto una volta sola; tutti i moduli riusano il valore globale
    while True:
        print_menu()
