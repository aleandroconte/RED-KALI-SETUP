...
# Proxy globale: impostato una volta sola al primo modulo che ne ha bisogno
PROXY: str = ""
...
def ask_proxy() -> str:
    """Chiede il proxy HTTP una volta sola; le chiamate successive riusano il valore globale."""
    global PROXY
    if PROXY:
        log(f"Proxy già configurato: {PROXY}")
        return PROXY
    print(f"\n{BOLD}Proxy HTTP{RESET} (es: http://10.80.254.254:3128 — lascia vuoto se non serve)")
    value = input("  Inserisci il proxy: ").strip()
    PROXY = value
    if PROXY:
...
def proxy_env(extra: dict = None) -> dict:
...
def curl(url: str, dest: str):
    """Esegue curl con proxy esplicito se configurato."""
